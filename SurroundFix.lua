...
sfixFrame:SetScript("OnEvent", function(self, event, ...) --This is essentially saying "On an event, run this function of everything below"
	...
	local fixX  = GetScreenWidth()--Get the Horizontal resolution of the setup
	local fixYdiv
	...
	function UIParentHook(self) --self is needed so it gets passed in on the hook
		...
		coreSet = true --Sets coreSet to true so it doesn't trigger from itsself
			...
			print("Hook Fired")
			uiResolution()
			print("Setting UI Resolution post res", floor(fixX+0.5),"x",floor(fixY+0.5))
			sfixAnnounce()
			print("Setting UI Resolution post announce", floor(fixX+0.5),"x",floor(fixY+0.5))
...
--		hooksecurefunc(UIParent, "SetScale", UIParentHook) --Hooks into UIParent "SetScale", so if anything tries to change that then it runs
    ...
    if event == "PLAYER_LOGIN" or event == "DISPLAY_SIZE_CHANGED" then --Main part of the code that runs when the events happen
		eventUnregister() --Unregister the events so it doesn't spam
		C_Timer.After(0.1, function() end) --Run the main code after 0.1 seconds to prevent issues

		C_Timer.After(rateLimit, function() eventRegister() end) --After the rateLimit amount of time, reregister the events, run the main code again, and print to the chat box
